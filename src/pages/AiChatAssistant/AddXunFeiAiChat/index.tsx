...
import {aiRecommendUsingPOST} from "@/services/sihaibi/aiIntelligentController";

const AddXunFeiAiChat: React.FC = () => {
  const [form] = useForm();
  ...
  const onFinish = async (values: any) => {
    ...
    try {
      const res = await aiRecommendUsingPOST(values);
      if (!res?.data) {
        message.error('操作失败' );
      } else {
...
export default AddXunFeiAiChat;
